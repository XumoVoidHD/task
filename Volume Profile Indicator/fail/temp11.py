...
# Set zero values of VAH, VAL, and POC to NaN
df['vah'] = df['vah'].replace(0, np.nan)
df['val'] = df['val'].replace(0, np.nan)
df['poc'] = df['poc'].replace(0, np.nan)
...
# Plot VAH, VAL, and POC with different line styles
fplt.plot(df['time'], df['vah'], style='v.', color='#FF0000', legend='VAH', ax=plot)  # Red dashed line
fplt.plot(df['time'], df['val'], style='<', color='#0000FF', legend='VAL', ax=plot)  # Blue dotted line
fplt.plot(df['time'], df['poc'], style='>', color='#00FF00', width=2, legend='POC',
          ax=plot)  # Green solid line, thicker

# Add horizontal volume profile to the same plot
for i, (time, vah, val, poc) in enumerate(volume_profile):
    if not np.isnan(vah) and not np.isnan(val) and not np.isnan(poc):
        profile_data = df[(df['time'] >= time) & (df['time'] < (time + pd.Timedelta(weeks=1)))]
        fplt.horiz_time_volume([(time, sorted(profile_data['hlc3'].tolist()))], draw_va=0.7, draw_poc=0.5, ax=plot)
